        ...
        try:
            ...
            resp = openai.chat.completions.create(
                ...
                messages=[
                    ...
                    {
                        'role': 'user',
                        'content': "complete the logical form, it's an expression using 'JOIN', 'ARGMIN', 'ARGMAX', 'R', 'le', 'lt', 'ge', 'gt', 'COUNT', 'AND', 'TC' and 'CONS' as keyword. Only give the expression directly without any explanation or clarification"
                    }
                ],
                temperature=temp,
                max_tokens=256,
...
def struct_of_exp(exp):
    ...
    for token in exp.split():
        if token in phrase_set:
            ans.append(token)
        elif not ans or ans[-1] != "x":
            ans.append("x")
    return ans

def mid_to_name(mids, mid_to_name_dict):
    ans = []
    for mid in mids:
        key = "/m/" + mid[2:]
        if key in mid_to_name_dict:
            ans.append(mid_to_name_dict[key])
...
def all_combiner_evaluation(data_batch, selected_quest_compare, selected_quest_compose, selected_quest,
                            ...
                            question_to_mid_dict, api_key, LLM_engine, name_to_id_dict, mid_to_name_dict, bm25_all_fns, all_fns,
        ...
        for idx, gene_exp in enumerate(scouts):
            try:
                ...
                for mid_iters in mid_combinations:
                    ...
                    logger.info("predicdted_mid: {}".format(answer))
                    answer = mid_to_name(answer, mid_to_name_dict)
    ...
    # with open("output/domains.summary", 'a') as f:
    #     f.write(f"{correct_exp}\t")
    #     f.write(f"{total_exp}")
    #     f.write("\n")
...
def parse_args():
    ...
    parser.add_argument('--mid_map_path', type=str, metavar='N',
                        default="data/mid2name.txt", help='mid2name file path')
...
def main():
    ...
    with open(args.mid_map_path) as f:
        lines = f.readlines()
    mid_to_name_dict = {}
    for line in lines:
        ...
        if len(info) < 2:
            continue
        mid, name = info
        if mid not in mid_to_name_dict:
            mid_to_name_dict[mid] = name

    # with open("output/domains.summary", 'a') as f:
    #     domain = args.train_data_path.split('/')[-1][:-5]
    #     f.write(f"{domain}\t")
    ...
    all_combiner_evaluation(dev_data, selected_quest_compose, selected_quest_compare, selected_quest, prompt_type,
                            ...
                            question_to_mid_dict, args.api_key, args.engine, name_to_id_dict, mid_to_name_dict, bm25_all_fns,

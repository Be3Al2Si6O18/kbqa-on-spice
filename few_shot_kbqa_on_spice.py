...
def sub_mid_to_fn(string, wikidata_mid_to_fn_dict):
    ...
    for seg in seg_list:
        if seg.startswith('P') or seg.startswith('Q'):
            mid = seg.strip(')(')
    ...
    for key in mid_to_start_idx_dict:
        ...
        e_idx = b_idx + len(key)
        # if key not in wikidata_mid_to_fn_dict:
        #     print(key)
        new_string = new_string + string[start_index:b_idx] + wikidata_mid_to_fn_dict.get(key, "unknown_entity")
        start_index = e_idx
...
def all_combiner_evaluation(data_batch, selected_quest_compare, selected_quest_compose, selected_quest,
                            ...
                            api_key, LLM_engine, name_to_id_dict, freebase_mid_to_fn_dict, wikidata_mid_to_fn_dict,
                            bm25_all_fns, all_fns, relationship_to_enti, retrieval=False, corpus=None,
                            nlp_model=None, bm25_train_full=None, retrieve_number=100):
    correct = [0] * 6
    total = [0] * 6
    no_ans = [0] * 6
    correct_exp = 0
        ...
        for idx, gene_exp in enumerate(scouts):
            try:
                ...
                for mid_iters in mid_combinations:
                    ...
                    answer = freebase_mid_to_fn(answer, freebase_mid_to_fn_dict)
...
def main():
    ...
    with open(args.freebase_map_path, 'rb') as f:
        freebase_mid_to_fn_dict = pickle.load(f)
    ...
    all_combiner_evaluation(dev_data, selected_quest_compose, selected_quest_compare, selected_quest, prompt_type,
                            ...
                            args.api_key, args.engine, name_to_id_dict,
                            freebase_mid_to_fn_dict, wikidata_mid_to_fn_dict, bm25_all_fns,
